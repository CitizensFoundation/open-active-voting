...
class PerformanceTest < ActionController::IntegrationTest
  def setup
    ...
    @max_browsers.times do
      @browsers << Watir::Browser.new(@browser_types[rand(@browser_types.length)], :profile => 'default')
    end
    @votes = []
  ...
  test "vote_through_firefox_and_chrome" do
    @votes.each do |vote|
      ...
      browser.goto "https://#{@http_auth}#{@base_host}/votes/authentication_options"
      browser.goto "https://innskraning.island.is/?id=#{@base_host}"
      browser.text_field(:id=>"ContentPlaceHolder1_SsnTextBox").set(ENV['OAV_TEST_SSN'])
      browser.text_field(:id=>"ContentPlaceHolder1_KeyBox").set(ENV['OAV_TEST_PWD'])
      login_button = browser.input(:id => "ContentPlaceHolder1_PwLoginBtn")
      login_button.when_present.click
      browser.goto "https://#{@http_auth}#{@base_host}/votes/get_ballot?area_id=#{area_id}"
      retry_count = 0
      user_votes = nil
      begin
