...
class BudgetBallotItem < ActiveRecord::Base
  ...
  def self.get_idea_name(area_id,idea_id)
    ...
    BudgetBallotItem.where(:budget_ballot_area_id=>area_id, :idea_id=>idea_id).first.name
  end

  def self.get_idea_description(area_id, idea_id)
    # Get the given idea description
    BudgetBallotItem.where(:budget_ballot_area_id=>area_id, :idea_id=>idea_id).first.description
  end

  def self.get_idea_link(area_id, idea_id)
    # Get the given idea link
    BudgetBallotItem.where(:budget_ballot_area_id=>area_id, :idea_id=>idea_id).first.link
  end
