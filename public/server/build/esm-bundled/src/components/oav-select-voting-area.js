import{html$1 as html,PageViewElement,unsafeHTML,OavShadowStyles,OavFlexLayout}from"./oav-app.js";class OavSelectVotingArea extends PageViewElement{static get properties(){return{configFromServer:{type:Object},hasLoadedCss:Boolean}}static get styles(){return[OavShadowStyles,OavFlexLayout]}b64DecodeUnicode(str){return decodeURIComponent(atob(str).split("").map(function(c){return"%"+("00"+c.charCodeAt(0).toString(16)).slice(-2)}).join(""))}renderX(){if(this.hasLoadedCss){return html`${this.wide?html`${unsafeHTML(this.setupHTMLText(this.configFromServer.client_config.selectVotingAreaDesktopHTML))}`:html`${unsafeHTML(this.setupHTMLText(this.configFromServer.client_config.selectVotingAreaMobileHTML))}`}`}else{return html``}}render(){if(this.hasLoadedCss){return html`${this.wide?html`${unsafeHTML(this.replaceInHTMLText(this.testHtml()))}`:html`${unsafeHTML(this.setupHTMLText(this.configFromServer.client_config.selectVotingAreaMobileHTML))}`}`}else{return html``}}testHtml(){return`
    <style>
  .selectedLanguage {
